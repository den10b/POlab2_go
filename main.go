...
func main() {
	...
	for i := 0; i < nRoutines; i++ {
		funcWG.Add(1)
		if useAsync {
			go brute(resArr, i*len(*resArr)/nRoutines, (i+1)*len(*resArr)/nRoutines, &toFind, &resChan, funcWG)
		} else {
			brute(resArr, i*len(*resArr)/nRoutines, (i+1)*len(*resArr)/nRoutines, &toFind, &resChan, funcWG)
		}
	}
	funcWG.Wait()
